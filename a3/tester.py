...
class Tester(threading.Thread):
    ...
    def __init__(self, data, name, params, checkMap):
        ...
        print(Color.highlight(ColorEscape.BRIGHT_MAGENTA, "Testing %s..." % name))
        self._initIpc()
...
def main():
    ...
    if args.docker or args.docker_persist:
        if not DOCKER_AVAILABLE:
            print(Color.highlight(ColorEscape.BRIGHT_RED, "Please install the docker module for Python"))
            sys.exit()
        containerArgs = []
        if args.verbose:
